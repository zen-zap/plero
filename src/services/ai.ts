...
export async function completionRag({
    ...
    if (cachedEmbeddings && cachedEmbeddings.length > 0 && cachedEmbeddings[0].length === 384) {
...
export function cosineSimilarity(embedding: number[], queryEmbedding: number[]): number {
    if(embedding.length != queryEmbedding.length) {
        console.log(`Length of embedding: ${embedding.length} and Length of queryEmbedding: ${queryEmbedding.length}`);
